...
static int cli_parse_set_cafile(char **args, char *payload, struct appctx *appctx, void *private)
{
	...
	appctx->ctx.ssl.ca_verify_instances = 0;
	...
	while (1) {
		switch (appctx->st2) {
			...
			case SETCERT_ST_GEN:
				...
				/* we didn't start yet, set it to the first elem */
				if (ckchi == NULL) {
					ckchi = LIST_ELEM(old_cafile_entry->ckch_inst.n, typeof(ckchi), by_cafile_entry);
					/* Add the newly created cafile_entry to the tree so that
					 * any new ckch instance created from now can use it. */
				...
				if (!appctx->ctx.ssl.ca_verify_instances) {
					/* walk through the old ckch_inst and create new ckch_inst using the updated cafile */
					list_for_each_entry_from(ckchi, &old_cafile_entry->ckch_inst, by_cafile_entry) {
						switch (__create_new_instance(appctx, ckchi, &y, trash, err)) {
					...
					/* We managed all the regular CA instances, we will now
					 * process the ca-verify ones (ca-verify-file option). */
					appctx->ctx.ssl.ca_verify_instances = 1;
					ckchi = LIST_ELEM(old_cafile_entry->verify_ckch_inst.n, typeof(ckchi), by_cavfile_entry);
				}

				/* walk through the old ckch_inst and create new ckch_inst using the updated cafile */
				list_for_each_entry_from(ckchi, &old_cafile_entry->verify_ckch_inst, by_cavfile_entry) {
					switch (__create_new_instance(appctx, ckchi, &y, trash, err)) {
					case CREATE_NEW_INST_YIELD:
						goto yield;
					case CREATE_NEW_INST_ERR:
			...
			case SETCERT_ST_INSERT:
				...
				/* insert the new ckch_insts in the crtlist_entry */
				list_for_each_entry(ckchi, &new_cafile_entry->ckch_inst, by_cafile_entry) {
					if (ckchi->crtlist_entry)
						LIST_INSERT(&ckchi->crtlist_entry->ckch_inst, &ckchi->by_crtlist_entry);
				}
				list_for_each_entry(ckchi, &new_cafile_entry->verify_ckch_inst, by_cavfile_entry) {
					if (ckchi->crtlist_entry)
						LIST_INSERT(&ckchi->crtlist_entry->ckch_inst, &ckchi->by_crtlist_entry);
				}

				/* First, we insert every new SNIs in the trees, also replace the default_ctx */
				list_for_each_entry_safe(ckchi, ckchis, &new_cafile_entry->ckch_inst, by_cafile_entry) {
					__ssl_sock_load_new_ckch_instance(ckchi);
				}
				list_for_each_entry_safe(ckchi, ckchis, &new_cafile_entry->verify_ckch_inst, by_cavfile_entry) {
					__ssl_sock_load_new_ckch_instance(ckchi);
				}

				/* delete the old sni_ctx, the old ckch_insts and the ckch_store */
				list_for_each_entry_safe(ckchi, ckchis, &old_cafile_entry->ckch_inst, by_cafile_entry) {
					__ckch_inst_free_locked(ckchi);
				}
				list_for_each_entry_safe(ckchi, ckchis, &old_cafile_entry->verify_ckch_inst, by_cavfile_entry) {
					__ckch_inst_free_locked(ckchi);
				}
...
static int cli_io_handler_show_cafile_detail(struct appctx *appctx)
{
	...
	if (!cafile_entry->ca_store)
		chunk_appendf(out, "Empty\n");
	else if (LIST_ISEMPTY(&cafile_entry->ckch_inst) && LIST_ISEMPTY(&cafile_entry->verify_ckch_inst))
		chunk_appendf(out, "Unused\n");
	else
		chunk_appendf(out, "Used\n");
...
static int cli_parse_del_cafile(char **args, char *payload, struct appctx *appctx, void *private)
{
	...
	if (!LIST_ISEMPTY(&cafile_entry->ckch_inst) || !LIST_ISEMPTY(&cafile_entry->verify_ckch_inst)) {
